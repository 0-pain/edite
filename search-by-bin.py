...
def handle_search(message):
    ...
    # استخراج الأرقام (مسافات أو أسطر)
    raw = message.text.strip()
    prefixes = [p.strip() for p in raw.replace("\n", " ").split(" ") if p.strip().isdigit()]

    if not prefixes:
        return bot.reply_to(message, "❌ أرسل أرقام فقط")

    # تحقق من الطول
    for p in prefixes:
        if not (6 <= len(p) <= 12):
            return bot.reply_to(message, "🔢 كل رقم يجب أن يكون من 6 إلى 12 خانة")

    bot.send_message(
        message.chat.id,
        f"🔍 جاري البحث عن {len(prefixes)} BIN / PREFIX ..."
    )
    ...
    if not results:
        return bot.send_message(message.chat.id, "❌ لا توجد نتائج")

    # حفظ النتائج
    out_file = os.path.join(DATA_DIR, f"RESULT_{uid}.txt")
    ...
    bot.send_document(
        message.chat.id,
        open(out_file, "rb"),
        caption=f"✅ تم العثور على {len(results)} نتيجة"
    )
